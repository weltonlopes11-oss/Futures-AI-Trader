...
def add_wilder_dmi_adx(frame: pd.DataFrame, period: int = ADX_PERIOD) -> pd.DataFrame:
    out = frame.copy()
    ...
    dx = 100.0 * (out["plus_di"] - out["minus_di"]).abs() / di_sum.replace(0.0, np.nan)
    out["adx"] = dx.ewm(alpha=1.0 / period, adjust=False, min_periods=period).mean()

    return out


def run_regime_adx_window(
    h1: pd.DataFrame,
    h4: pd.DataFrame,
    eval_start: datetime,
    eval_end: datetime,
    ...
    enriched = enrich_indicators(h1, cfg)
    enriched = add_wilder_dmi_adx(enriched, ADX_PERIOD)
    ...
    for i, row in enriched.iterrows():
        ...
        if in_eval and position is None and pending is None and i + 1 < len(enriched):
            ...
            if bool(row["long_signal"]):
                ...
                elif pd.isna(adx) or float(adx) <= ADX_THRESHOLD:
                    counts["long"]["blocked_adx"] += 1
                elif pd.isna(plus_di) or pd.isna(minus_di) or float(plus_di) <= float(minus_di):
                    counts["long"]["blocked_di"] += 1
                else:
...
def main() -> None:
    ...
    candidate, combined_counts = run_regime_adx_window(h1, h4, eval_start, eval_end, cfg)

    result = {
        "experiment_id": "4h-ema50-plus-1h-adx14-dmi-jan-aug-2026",
        ...
        "candidate_change_only_vs_regime_4h": {
            "dmi_adx_timeframe": "1h completed signal candle",
            "period": ADX_PERIOD,
            "adx_threshold": ADX_THRESHOLD,
            "long_gate": "+DI > -DI and ADX > 20",
            "short_gate": "-DI > +DI and ADX > 20",
            "smoothing": "Wilder RMA via alpha=1/14",
            "causality": "DMI/ADX computed only from completed 1h candles; accepted signal enters next 1h open",
        },
        "unchanged": {
