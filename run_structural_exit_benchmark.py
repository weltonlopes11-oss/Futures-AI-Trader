...
def main():
    ...
    fixed_engine = OperationalBacktest(fee_bps_per_side=fee, slippage_bps_per_side=slippage)
    ...
    out = Path("artifacts/structural-exit-benchmark")
    out.mkdir(parents=True, exist_ok=True)

    rows = []
    for rr in (1.0, 1.5, 2.0, 3.0):
        for mode, engine in (
            ("keltner_fixed_exit_control", fixed_engine),
            ("keltner_structure_exit_5bar", structure_engine),
        ):
            trades = engine.run(
                keltner_entries,
                keltner_entries[["timestamp", "decision"]],
                rr=rr,
            )
            met = engine.metrics(trades)
            structure_exits = 0 if trades.empty else int((trades["outcome"] == "STRUCTURE_EXIT").sum())
            met.update({"mode": mode, "rr": rr, "structure_exits": structure_exits})
            rows.append(met)
    ...
    manifest = {
        "benchmark": "structural-exit-v1",
        "entry": "frozen core + Keltner battery 1",
        "control_exit": "1 ATR stop + fixed R target",
        "candidate_exit": "same stop/target plus adverse 5-bar structure break at candle close",
        "structure_lookback_bars": 5,
        "structure_horizon_minutes": 75,
        "long_exit": "close < lowest low of prior 5 completed 15m bars",
        "short_exit": "close > highest high of prior 5 completed 15m bars",
        "priority": "stop/target first intrabar; structure exit second at close",
        "symbol": symbol,
