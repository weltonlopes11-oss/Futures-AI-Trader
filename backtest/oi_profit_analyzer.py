...
class OIProfitAnalyzer:
    """
    Mede o potencial econômico dos impulsos de Open Interest.

    A análise é deliberadamente separada do mecanismo de decisão: ela não altera
    filtros, scores, thresholds ou execução. Os percentis do OI são derivados do
    próprio histórico e os retornos futuros são medidos em múltiplos horizontes.

    MFE (Maximum Favorable Excursion): maior deslocamento favorável dentro da
    janela futura.

    MAE (Maximum Adverse Excursion): maior deslocamento adverso dentro da
    janela futura.
    """
    ...
    def _build_base_observations(self) -> list[dict[str, Any]]:
        ...
        for record in self.records:
            ...
            # O OI de 5m é carregado para frente nos candles de 1m.
            # Só tratamos como nova observação quando o valor muda.
    ...
    def _build_profit_observations(self) -> list[OIProfitObservation]:
        if not self.base_observations:
            return []

        indexed = self.candles.set_index("timestamp")
        ...
        for item in self.base_observations:
            ...
            current = indexed.loc[timestamp]
            if isinstance(current, pd.DataFrame):
                current = current.iloc[-1]

            entry_price = self._number(current["close"])
            if entry_price in (None, 0):
                continue

            for horizon in self.horizons:
                end_time = timestamp + pd.Timedelta(minutes=horizon)
                future = self.candles.loc[
                    (self.candles["timestamp"] > timestamp)
                    & (self.candles["timestamp"] <= end_time)
                ]

                # Só usamos janelas completas. Isso evita enviesar o final do dataset.
                ...
                if None in (exit_price, high_price, low_price):
                    continue
    ...
    def summary_by_bucket(self) -> dict[int, dict[str, dict[str, float | int]]]:
        grouped: dict[int, dict[str, list[OIProfitObservation]]] = defaultdict(
        ...
        return {
            horizon: {
                bucket: self._summarize(items)
                for bucket, items in buckets.items()
            }
            for horizon, buckets in grouped.items()
        }

    def summary_by_decision_and_bucket(
        self,
    ) -> dict[int, dict[str, dict[str, dict[str, float | int]]]]:
        grouped: dict[int, dict[str, dict[str, list[OIProfitObservation]]]] = defaultdict(
    ...
    def summary_by_regime_and_bucket(
        self,
    ) -> dict[int, dict[str, dict[str, dict[str, float | int]]]]:
        grouped: dict[int, dict[str, dict[str, list[OIProfitObservation]]]] = defaultdict(
            lambda: defaultdict(lambda: defaultdict(list))
    ...
    def _summarize(items: list[OIProfitObservation]) -> dict[str, float | int]:
        if not items:
            return {
                ...
                "median_directional_return_pct": 0.0,
        ...
        returns = sorted(item.directional_return_pct for item in items)
        mfes = [item.mfe_pct for item in items]
        maes = [item.mae_pct for item in items]

        n = len(items)
        average_return = sum(returns) / n
        average_mfe = sum(mfes) / n
        average_mae = sum(maes) / n
        positive_rate = sum(1 for value in returns if value > 0) / n * 100.0

        if n % 2:
            median_return = returns[n // 2]
        else:
            median_return = (returns[n // 2 - 1] + returns[n // 2]) / 2.0

        return {
            "count": n,
            "avg_directional_return_pct": round(average_return, 6),
            "median_directional_return_pct": round(median_return, 6),
            "positive_rate_pct": round(positive_rate, 2),
            "avg_mfe_pct": round(average_mfe, 6),
            "avg_mae_pct": round(average_mae, 6),
            "mfe_mae_ratio": round(
