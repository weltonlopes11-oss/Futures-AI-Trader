...
class OpenInterestEnricher:
    ...
    def enrich(self, candles: pd.DataFrame, open_interest: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing:
            raise ValueError(f"Open Interest inválido. Colunas ausentes: {sorted(missing)}")
        ...
        right["open_interest_source_timestamp"] = right["timestamp"]
        right["open_interest"] = pd.to_numeric(right["sumOpenInterest"], errors="raise")
        right["open_interest_value"] = pd.to_numeric(right["sumOpenInterestValue"], errors="raise")
        right = (
            right[["timestamp", "open_interest_source_timestamp", "open_interest", "open_interest_value"]]
            .drop_duplicates(subset="timestamp")
            .sort_values("timestamp")
            .reset_index(drop=True)
        )
        right["open_interest_change_pct"] = right["open_interest"].pct_change() * 100.0
    ...
    def from_binance_records(records: list[dict]) -> pd.DataFrame:
        ...
        df["timestamp"] = pd.to_datetime(
            pd.to_numeric(df["timestamp"], errors="raise"), unit="ms"
