...
def run_regime_atr_price_window(
    h1: pd.DataFrame,
    h4: pd.DataFrame,
    eval_start: datetime,
    eval_end: datetime,
    ...
    counts = {
        "long": {"accepted": 0, "blocked_4h": 0, "blocked_atr_price": 0},
        "short": {"accepted": 0, "blocked_4h": 0, "blocked_atr_price": 0},
    }
    ...
    for i, row in enriched.iterrows():
        ...
        if position is None and pending is not None and in_eval:
            ...
            if pd.isna(prior_atr):
                ...
                position = {
                    ...
                    "entry_atr": atr,
                    "entry_atr_price_ratio": pending["atr_price_ratio"],
                    "stop_price": (
        ...
        if in_eval and position is None and pending is None and i + 1 < len(enriched):
            ...
            atr = row.get("keltner_atr")
            close = row.get("close")
            atr_price_ratio = float("nan")
            if pd.notna(atr) and pd.notna(close) and float(close) != 0.0:
                atr_price_ratio = float(atr) / float(close)

            if bool(row["long_signal"]):
                if regime != "BULLISH":
                    counts["long"]["blocked_4h"] += 1
                elif pd.isna(atr_price_ratio) or atr_price_ratio > ATR_PRICE_MAX:
                    counts["long"]["blocked_atr_price"] += 1
                else:
                ...
                if regime != "BEARISH":
                    ...
                    counts["short"]["blocked_atr_price"] += 1
                else:
...
def main() -> None:
    ...
    candidate, combined_counts = run_regime_atr_price_window(h1, h4, eval_start, eval_end, cfg)

    result = {
        "experiment_id": "4h-ema50-plus-1h-atr20-price-max-1pct-jan-aug-2026",
        ...
        "candidate_change_only_vs_regime_4h": {
            "timeframe": "1h completed signal candle",
            "atr_period": 20,
            "max_atr_price_ratio": ATR_PRICE_MAX,
            "gate": "ATR20 / close <= 1.0%",
            "risk_interpretation": "with 2ATR stop and 10x exposure, threshold implies roughly <=20% gross equity move to stop before costs/gaps",
            "causality": "uses only completed 1h signal candle; accepted signal enters next 1h open",
        },
        "unchanged": {
        ...
        "regime_4h_plus_atr_price_max": metrics(candidate),
        "official_leveraged_path": leveraged_path(official),
        "regime_4h_only_leveraged_path": leveraged_path(regime_only),
        "regime_4h_plus_atr_price_max_leveraged_path": leveraged_path(candidate),
